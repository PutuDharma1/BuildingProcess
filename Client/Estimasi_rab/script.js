...
let isInitialized = false; // Tambahkan flag untuk mencegah inisialisasi berulang
let isCalculating = false; // Tambahkan flag untuk mencegah recursive calculation

const sipilCategories = [
  "PEKERJAAN PERSIAPAN",
...
const formatNumberWithSeparators = (num) =>
  num === null || isNaN(num) ? "0" : new Intl.NumberFormat("id-ID").format(num);
...
const populateJenisPekerjaanOptionsForNewRow = (rowElement) => {
  ...
  let dataSource =
    scope === "Sipil" && categorizedPrices.categorizedSipilPrices
  ...
  if (filteredItems.length === 0) {
    selectEl.innerHTML =
      '<option value="">-- Data Harga Tidak Ditemukan --</option>';
    return;
  }
...
const autoFillPrices = (selectElement) => {
  ...
  if (dataSource?.[currentCategory]) {
    selectedItem = dataSource[currentCategory].find(
      (item) =>
        item["Jenis Pekerjaan"] === selectedJenisPekerjaan &&
  ...
  if (selectedItem) {
    ...
    if (selectedItem["Harga Material"] === "Kondisional") {
      ...
      materialPriceInput.value = formatNumberWithSeparators(
        selectedItem["Harga Material"]
      );
    ...
    if (selectedItem["Harga Upah"] === "Kondisional") {
      ...
      upahPriceInput.value = formatNumberWithSeparators(
        selectedItem["Harga Upah"]
      );
...
const createBoQRow = (category, scope) => {
  ...
  row.innerHTML = `
        ...
        <td class="col-jenis-pekerjaan">
            <select class="jenis-pekerjaan form-control" name="Jenis_Pekerjaan_Item" required>
                <option value="">-- Pilih --</option>
            </select>
        </td>
        <td class="col-satuan">
            <input type="text" class="satuan form-control" name="Satuan_Item" required readonly />
        </td>
        <td class="col-volume">
        ...
        <td class="col-harga">
            <input type="text" class="harga-material form-control" name="Harga_Material_Item" inputmode="numeric" required readonly />
        </td>
        <td class="col-harga">
            <input type="text" class="harga-upah form-control" name="Harga_Upah_Item" inputmode="numeric" required readonly />
        </td>
        <td class="col-harga">
            <input type="text" class="total-material form-control" disabled />
        </td>
        <td class="col-harga">
            <input type="text" class="total-upah form-control" disabled />
        </td>
        <td class="col-harga">
            <input type="text" class="total-harga form-control" disabled />
        </td>
        <td class="col-aksi">
  ...
  row
    ...
    .addEventListener("input", (e) => calculateTotalPrice(e.target));
  row.querySelector(".delete-row-btn").addEventListener("click", () => {
    row.remove();
    updateAllRowNumbersAndTotals();
  });
  row
    .querySelector(".jenis-pekerjaan")
    .addEventListener("change", (e) => autoFillPrices(e.target));
...
const updateAllRowNumbersAndTotals = () => {
  ...
  try {
    document
      .querySelectorAll(".boq-table-body:not(.hidden)")
      .forEach((tbody) => {
        tbody.querySelectorAll(".boq-item-row").forEach((row, index) => {
          row.querySelector(".row-number").textContent = index + 1;
          calculateTotalPriceForRow(row); // Use separate function to avoid recursion
        });
...
// Separate function to calculate total price for a specific row
const calculateTotalPriceForRow = (row) => {
  const volume = parseFloat(row.querySelector("input.volume").value) || 0;
  const material = parseFormattedNumber(
    row.querySelector("input.harga-material").value
  );
  const upah = parseFormattedNumber(
    row.querySelector("input.harga-upah").value
  );
  ...
  row.querySelector("input.total-harga").value = formatRupiah(
    totalMaterial + totalUpah
  );
};
...
const calculateSubTotal = (tbodyElement) => {
  let subTotal = 0;
  tbodyElement
    .querySelectorAll(".boq-item-row .total-harga")
    .forEach((input) => (subTotal += parseRupiah(input.value)));

  const subTotalAmountElement = tbodyElement
    .closest("table")
    .querySelector(".sub-total-amount");
...
function calculateTotalPrice(inputElement) {
  ...
  // Update subtotal and grand total without recursion
  const tbody = row.closest(".boq-table-body");
  if (tbody) {
    calculateSubTotal(tbody);
    calculateGrandTotal();
  }
...
const calculateGrandTotal = () => {
  let total = 0;
  document
    .querySelectorAll(".boq-table-body:not(.hidden) .total-harga")
    .forEach((input) => (total += parseRupiah(input.value)));
...
const populateFormWithHistory = (data) => {
  ...
  form.reset();
  document
    .querySelectorAll(".boq-table-body")
    .forEach((tbody) => (tbody.innerHTML = ""));

  const lingkupPekerjaanValue =
    data["Lingkup_Pekerjaan"] || data["Lingkup Pekerjaan"];
  ...
  for (const key in data) {
    if (data.hasOwnProperty(key)) {
      ...
      if (
        element &&
        key !== "Lingkup_Pekerjaan" &&
        key !== "Lingkup Pekerjaan"
      ) {
        element.value =
          element.type === "date" && data[key]
  ...
  setTimeout(() => {
      ...
      if (data[`Jenis_Pekerjaan_${i}`]) {
        ...
        if (targetTbody) {
          ...
          newRow.querySelector(".jenis-pekerjaan").value =
            data[`Jenis_Pekerjaan_${i}`];
          autoFillPrices(newRow.querySelector(".jenis-pekerjaan"));
          newRow.querySelector(".volume").value = parseFloat(
            data[`Volume_Item_${i}`] || 0
          ).toFixed(2);

          if (newRow.querySelector(".harga-material").readOnly === false) {
            newRow.querySelector(".harga-material").value =
              formatNumberWithSeparators(data[`Harga_Material_Item_${i}`]);
          }
          if (newRow.querySelector(".harga-upah").readOnly === false) {
            newRow.querySelector(".harga-upah").value =
              formatNumberWithSeparators(data[`Harga_Upah_Item_${i}`]);
          }
        }
      }
    }
    ...
    messageDiv.innerHTML =
      "Data revisi telah dimuat. Silakan lakukan perubahan dan klik 'Kirim' jika sudah selesai.";
...
async function handleFormSubmit() {
  ...
  const PYTHON_API_BASE_URL = "https://buildingprocess-fld9.onrender.com"; // Fixed URL
  const requiredFields = ["Lokasi", "Proyek", "Cabang", "Lingkup Pekerjaan"];

  for (const fieldName of requiredFields) {
    const element = form.elements[fieldName];
    if (!element || !element.value.trim()) {
      messageDiv.textContent = `Error: Field '${fieldName.replace(
        /_/g,
        " "
      )}' wajib diisi.`;
      messageDiv.style.display = "block";
  ...
  if (
    ...
    (!lastRejectedSubmission ||
      currentStoreCode !== String(lastRejectedSubmission.Lokasi).toUpperCase())
  ) {
  ...
  try {
    ...
    document
      ...
      .forEach((row) => {
        ...
        if (jenisPekerjaanInput && jenisPekerjaanInput.value) {
          itemCounter++;
          formDataToSend[`Kategori_Pekerjaan_${itemCounter}`] =
            row.dataset.category;
          formDataToSend[`Jenis_Pekerjaan_${itemCounter}`] =
            jenisPekerjaanInput.value;
          formDataToSend[`Satuan_Item_${itemCounter}`] =
            row.querySelector(".satuan").value;
          formDataToSend[`Volume_Item_${itemCounter}`] =
            parseFloat(row.querySelector(".volume").value) || 0;
          formDataToSend[`Harga_Material_Item_${itemCounter}`] =
            parseFormattedNumber(row.querySelector(".harga-material").value);
          formDataToSend[`Harga_Upah_Item_${itemCounter}`] =
            parseFormattedNumber(row.querySelector(".harga-upah").value);
          formDataToSend[`Total_Material_Item_${itemCounter}`] = parseRupiah(
            row.querySelector(".total-material").value
          );
          formDataToSend[`Total_Upah_Item_${itemCounter}`] = parseRupiah(
            row.querySelector(".total-upah").value
          );
          formDataToSend[`Total_Harga_Item_${itemCounter}`] = parseRupiah(
    ...
    if (itemCounter === 0) {
      throw new Error(
        "Tidak ada item pekerjaan yang ditambahkan. Formulir tidak bisa dikirim."
      );
    }
    ...
    messageDiv.textContent =
      data.message || "Data berhasil terkirim! Anda akan diarahkan ke Beranda.";
...
function createTableStructure(categoryName, scope) {
  ...
  tableWrapper.innerHTML = `
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th class="col-jenis-pekerjaan" rowspan="2">Jenis Pekerjaan</th>
                        <th class="col-satuan" rowspan="2">Satuan</th>
                        <th class="col-volume" rowspan="2">Volume</th>
                        <th class="col-harga" colspan="2">Harga Satuan (Rp)</th>
                        <th class="col-harga" colspan="2">Total Harga (Rp)</th>
                        <th class="col-harga" rowspan="2">Total Harga (Rp)</th>
                        <th class="col-aksi" rowspan="2">Aksi</th>
                    </tr>
                    <tr>
                        <th class="col-harga">Material</th>
                        <th class="col-harga">Upah</th>
                        <th class="col-harga">Material</th>
                        <th class="col-harga">Upah</th>
                    </tr>
                </thead>
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="sub-total-amount" style="font-weight: bold; text-align: right">Rp 0</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
        ...
        <div class="add-row-button-container">
            <button type="button" class="add-row-btn" data-category="${categoryName}" data-scope="${scope}">Tambah Item</button>
        </div>
    `;
  return tableWrapper;
}
...
async function initializePage() {
  ...
  try {
    ...
    sipilCategories.forEach((category) =>
      sipilTablesWrapper.appendChild(createTableStructure(category, "Sipil"))
    );
    meCategories.forEach((category) =>
      meTablesWrapper.appendChild(createTableStructure(category, "ME"))
    );

    const PYTHON_API_BASE_URL = "https://buildingprocess-fld9.onrender.com"; // Fixed URL
    const APPS_SCRIPT_DATA_URL =
      "https://script.google.com/macros/s/AKfycbx2rtKmaZBb_iRBRL-DOemjVhAp3GaCwsthtwtfdtvdtuO2bRVlmONboB8wE-CZU7Hc/exec";
    const userEmail = sessionStorage.getItem("loggedInUserEmail");

    const statusPromise = userEmail
      ? fetch(
          `${PYTHON_API_BASE_URL}/api/check_status?email=${encodeURIComponent(
            userEmail
          )}`
        ).then((res) => res.json())
    ...
    const [statusResult, pricesData] = await Promise.all([
      statusPromise,
      pricesPromise,
    ]);
    ...
    if (statusResult) {
      ...
      if (statusResult.rejected_submissions) {
        ...
        if (rejectedSubmissionsList.length > 0) {
          const rejectedCodes = rejectedSubmissionsList
    ...
    // Add event listeners (remove existing ones first to prevent duplication)
    document
      .getElementById("lokasi")
      ?.removeEventListener("input", handleLokasiInput);
    document
      .getElementById("lokasi")
      ?.addEventListener("input", handleLokasiInput);

    // Remove existing event listeners for add-row buttons
    document.querySelectorAll(".add-row-btn").forEach((button) => {
      button.removeEventListener("click", handleAddRowClick);
      button.addEventListener("click", handleAddRowClick);
    });
    ...
    lingkupPekerjaanSelect.removeEventListener(
      "change",
      handleLingkupPekerjaanChange
    );
    lingkupPekerjaanSelect.addEventListener(
      "change",
      handleLingkupPekerjaanChange
    );

    cabangSelect.removeEventListener("change", refreshAllDropdowns);
    cabangSelect.addEventListener("change", refreshAllDropdowns);

    currentResetButton.removeEventListener("click", handleResetClick);
    currentResetButton.addEventListener("click", handleResetClick);
...
// Separate event handler functions to prevent duplication
function handleLokasiInput() {
  const currentStoreCode = this.value.toUpperCase();
  const rejectedData = rejectedSubmissionsList.find(
    (sub) => String(sub.Lokasi).toUpperCase() === currentStoreCode
  );
  if (rejectedData) {
    if (
      confirm(
        "Data pengajuan yang ditolak untuk kode toko ini ditemukan. Apakah Anda ingin memuat ulang data tersebut untuk direvisi?"
      )
    ) {
...
function handleLingkupPekerjaanChange(event) {
...
function handleResetClick() {
  if (
    confirm(
      "Apakah Anda yakin ingin mengulang dan mengosongkan semua isian form?"
    )
  ) {
...
// Use DOMContentLoaded instead of pageshow to prevent multiple initialization
