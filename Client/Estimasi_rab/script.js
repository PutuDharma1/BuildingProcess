    ...
    for (const key in data) {
        if (data.hasOwnProperty(key)) {
            ...
            if (element && key !== 'Lingkup_Pekerjaan' && key !== 'Lingkup Pekerjaan') {
                element.value = (element.type === 'date' && data[key]) ? 
                    new Date(data[key]).toISOString().split('T')[0] : data[key];
            }
        }
    }
...
async function handleFormSubmit() {
    ...
    if (pendingStoreCodes.map(code => String(code).toUpperCase()).includes(currentStoreCode) && 
        (!lastRejectedSubmission || currentStoreCode !== String(lastRejectedSubmission.Lokasi).toUpperCase())) {
...
async function initializePage() {
    ...
    try {
        ...
        const statusPromise = userEmail ? 
            fetch(`${PYTHON_API_BASE_URL}/api/check_status?email=${encodeURIComponent(userEmail)}`)
                .then(res => res.json()) : 
        ...
        document.getElementById('lokasi')?.removeEventListener('input', handleLokasiInput);
        document.getElementById('lokasi')?.addEventListener('input', handleLokasiInput);
        ...
        lingkupPekerjaanSelect.removeEventListener("change", handleLingkupPekerjaanChange);
        lingkupPekerjaanSelect.addEventListener("change", handleLingkupPekerjaanChange);
...
function handleLokasiInput() {
    ...
    const rejectedData = rejectedSubmissionsList.find(sub => 
        String(sub.Lokasi).toUpperCase() === currentStoreCode
    );
    if (rejectedData) {
...
>>>>>>> f0e23e0dc35d62a09aba44917ad5a731dcaef602
