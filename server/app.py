## file: server/app.py (Lengkap & Final)
...
def submit_form():
    ...
    try:
        ...
        if not cabang: raise Exception(f"Field 'Cabang' is empty. Cannot find Coordinator.")
        coordinator_email = google_provider.get_email_by_jabatan(cabang, config.JABATAN.KOORDINATOR)
        if not coordinator_email: raise Exception(f"Coordinator email for branch '{cabang}' not found. Please check the 'Cabang' sheet.")
...
def handle_approval():
    ...
    # Membuat URL absolut untuk logo
    ...
    try:
        ...
        if action == 'reject':
            ...
            if manager_email:
                ...
                approval_url_manager = f"{base_render_url}/handle_approval?action=approve&row={row}&level=manager&approver={manager_email}"
                rejection_url_manager = f"{base_render_url}/handle_approval?action=reject&row={row}&level=manager&approver={manager_email}"
